...
from Lisa.modules.alive import Mukesh
import Lisa.modules.sql.userinfo_sql as sql
from Lisa import (
    DEMONS,
    DEV_USERS,
    DRAGONS,
...
from Lisa.__main__ import STATS, TOKEN, USER_INFO
from Lisa.modules.disable import DisableAbleCommandHandler
from Lisa.modules.helper_funcs.chat_status import sudo_plus
from Lisa.modules.helper_funcs.extraction import extract_user
from Lisa.modules.sql.global_bans_sql import is_user_gbanned
from Lisa.modules.sql.users_sql import get_user_num_chats
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nᴛʜɪs ᴜsᴇʀ ɪs ᴀ ᴍᴇᴍʙᴇʀ ᴏғ <b>PʀᴏBᴏᴛs</b>.\n"
        disaster_level_present = True
    ...
    if disaster_level_present:
        text += ' \n[<a href="https://t.me/LisaBotHQ/5">ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴋɴᴏᴡ ᴡʜᴀᴛ ɪs ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟs.</a>]'.format(
            bot.username
        )
            ...
            message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url=f"https://t.me/LisaBotHQ/4"
                            ),
                            InlineKeyboardButton(
                                "Dɪsᴀsᴛᴇʀ", url="https://t.me/LisaBotHQ/5"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
            ...
            message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url="https://t.me/LisaBotHQ/4"
                            ),
                            InlineKeyboardButton(
                                "ᴅɪsᴀsᴛᴇʀ", url="https://t.me/LisaBotHQ/5"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "ᴜᴍᴍ... ʏᴇᴀʜ, ɪ ᴏɴʟʏ ᴛʀᴜsᴛ PʀᴏBᴏᴛs ᴛᴏ sᴇᴛ ᴍʏ ʙɪᴏ."
            )
            return
